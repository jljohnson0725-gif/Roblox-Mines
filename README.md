...
The world builds itself at runtime: ground, spawn, and twelve plots are all
generated by `PlotService`, so the place file contains nothing but scripts.
Note it **removes Studio's default Baseplate** if present — the plot row is
~800 studs long and overruns it, and two coplanar ground slabs z-fight.
